...
description: Production failure semantics: timeouts, bounded retries, circuit breakers, back pressure, and observability. Use for outbound calls, queues, workers, and webhooks - the email outbox worker, PayMongo webhook reconciliation, and SMTP sends. Covers what happens when a dependency is slow, partial, or down.
---
...
Source: [ciembor/agent-rules-books](https://github.com/ciembor/agent-rules-books) (Release It! by Michael T. Nygard), MIT licensed. Verbatim `mini` edition.
